...
def youtube_search(query, n=10):
    ...
    if not exe: return []
    try:
        p = subprocess.run([exe,"--skip-download","--no-playlist","--flat-playlist","--print","%(id)s\\t%(title)s\\t%(channel)s\\t%(uploader)s",f"ytsearch{n}:{query}"], capture_output=True, text=True, timeout=45)
